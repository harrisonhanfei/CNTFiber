...
int Input::Read_rve_geometry(struct Geom_RVE &geom_rve, ifstream &infile)
{
	...
	//Define the shape of RVE
	istringstream istr0(Get_Line(infile));
	istr0 >> geom_rve.shape;
	if(geom_rve.shape!="Cuboid"&&geom_rve.shape!="Cylinder")
	{
		cout << "Error: the shape of RVE does not match the default values!" << endl;
		hout << "Error: the shape of RVE does not match the default values!" << endl;
		return 0;
	}
	...
	{
		istr0 >> geom_rve.cyl_radius >> geom_rve.cyl_length >> geom_rve.cyl_shell_thick >> geom_rve.cyl_hollow_rad;
		if(geom_rve.cyl_radius<=0||geom_rve.cyl_length<=0||geom_rve.cyl_hollow_rad<=0||geom_rve.cyl_shell_thick<=0)
		{
			cout << "Error: the sizes of RVE should be positive!" << endl;
			hout << "Error: the sizes of RVE should be positive!" << endl;
			return 0;
